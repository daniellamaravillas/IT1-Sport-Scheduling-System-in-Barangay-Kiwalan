...
try {
    // Retrieve StatusID for "schedule" from Updated_Status table (insert if missing)
    $stmt = $conn->prepare("SELECT StatusID FROM Updated_Status WHERE updated_status = ? LIMIT 1");
    $acceptedStatus = 'schedule';  // changed from "confirm" to "schedule"
    $stmt->bind_param("s", $acceptedStatus);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows === 0) {
        $stmt->close();
    ...
    // Update the pending schedule to accepted (now "schedule")
    ...
    // Redirect to calendar after changing pending to schedule
